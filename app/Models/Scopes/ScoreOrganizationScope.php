...
/**
 * Restricts queries to rows whose parent submission's registration's category's
 * competition belongs to the actor's organization.
 */
class ScoreOrganizationScope implements Scope
{
    public function __construct(
        private readonly string $submissionForeignKey = 'submission_id',
    ) {}
    ...
    public function apply(Builder $builder, Model $model): void
    {
        ...
        $foreignKey = $this->submissionForeignKey;

        $builder->whereExists(function ($query) use ($table, $foreignKey, $user): void {
            $query->selectRaw('1')
                ->from('submissions')
                ->join('registrations', 'registrations.id', '=', 'submissions.registration_id')
                ...
                ->whereColumn('submissions.id', "{$table}.{$foreignKey}")
